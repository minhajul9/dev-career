...
const Job = ({ job, children }) => {
    let i = 0;
    const { job_title, location, job_type, company, salary, image_url } = job;
    return (
        <div className={`p-10 rounded-md border flex ${children}`}>
            <div className={`${children.includes('flex-row') && 'w-56'}`}>
                <img src={image_url} className='h-10 mb-8' alt="" />
            </div>

            <div className={children.includes('flex-row')&& 'w-full flex justify-between items-center'}>
                <div>
